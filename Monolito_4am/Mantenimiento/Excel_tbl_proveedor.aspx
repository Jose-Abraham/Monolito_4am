<%@ Page Title="" Language="C#" MasterPageFile="~/Mantenimiento/Principal.Master" AutoEventWireup="true" CodeBehind="Excel_tbl_proveedor.aspx.cs" Inherits="Monolito_4am.Mantenimiento.Excel_tbl_proveedor" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <style>
        .import-card { max-width: 1100px; margin: auto; background: rgba(255,255,255,0.04); 
            border-radius: 20px; padding: 30px; }
        .title { text-align: center; color: #38bdf8; font-size: 28px; margin-bottom: 25px; }
        .custom-input { width: 100%; padding: 12px; border-radius: 12px; background: rgba(255,255,255,0.08); 
    ...
    <div class="import-card">
        <div class="title">Importar Proveedores desde Excel / CSV</div>
        ...
        <asp:Button ID="btnLimpiarPreview" runat="server"
            Text="Limpiar Previsualización"
            CssClass="btn"
            Style="background:#ef4444; color:white; margin-left:10px;"
            OnClick="btnLimpiarPreview_Click" />
        ...
        <asp:GridView ID="gvPreview" runat="server" CssClass="grid" AutoGenerateColumns="True" 
            GridLines="None" AllowPaging="false"></asp:GridView>
    </div>
...
<script>
    function limpiarArchivo() {
        var file = document.getElementById('<%= fuExcel.ClientID %>');
    if (file) {
        file.value = '';
    }
}
